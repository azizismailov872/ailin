...
const ListContainer = () => {
    ...
    useEffect(() => {
        ...
        document.title = 'Администраторы';
    }, []);

    const { users, total, per_page, current_page, isLoaded } = useSelector(
        (state) => ({
            users: state.adminUsers.users,
            total: state.adminUsers.total,
            current_page: state.adminUsers.current_page,
            per_page: state.adminUsers.per_page,
            isLoaded: state.adminUsers.isLoaded,
        })
    );
    ...
    const onDeletePhoto = async (id) => {
        setFetching(true);
        let response = await dispatch(deleteUserPhoto(id));
    ...
    return !isLoaded ? (
        ...
        <List
            ...
            per_page={per_page}
            updateLink="/admin/admin-users/update/"
            createLink="/admin/admin-users/create"
            title="Администраторы"
            onReset={onReset}
            successMessage="Пользователь удален"
            deletePhoto={deletePhoto}
            closeDeletePhoto={closeDeletePhoto}
